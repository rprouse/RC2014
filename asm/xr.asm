...
; https://github.com/coopzone-dc/xmodem80
;
; Bug fix: Stack overflow error, by D.Cooper 2020
;     Also some code rearranged to use call instead of jp for BDOS functions
; 22/04/23 Derek Cooper, Fixed some issues with code
; thanks for the suggestions from Wayne Warthen and Richard Deane.
;
;
...
;
	...
	ld	(vCONST+2),A	; Update jump target addressed
	ld	(vCONIN+2),A	; ...
	ld	(vCONOUT+2),A	; ...
...
Done:
	...
	call	CONOUT
        ld      A,2             ; this is a fidle, waits and clears input
        call    GetCharTmo
...
CONST:
	...
	push	BC
vCONST:	EQU	$
	call	0ff06h	; A=0 if no character is ready, 0FFh if one is
	pop	BC
	pop	HL
	ret
...
CONIN:
	...
	push	BC
vCONIN:	equ	$
	call	0ff09h	; Wait until character ready available and return in A
	pop	BC
	pop	HL
	ret
...
CONOUT:
	...
	push	BC
vCONOUT:equ	$
	call	0ff0ch	; Write the character in C to the screen
	pop	BC
	pop	HL
	ret
;
; Message strings
;
msgHeader: DB 	'CP/M XR - Xmodem receive v1.0',CR,LF,0
